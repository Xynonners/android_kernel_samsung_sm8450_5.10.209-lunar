...
#if IS_ENABLED(CONFIG_OPLUS_SCHED_PIPELINE)
...
#if IS_ENABLED(CONFIG_OPLUS_SCHED_LOADBALANCE)
	cpumask_t oplus_cpu_array[2*OPLUS_MAX_CLS][OPLUS_MAX_CLS];
#endif
};

#if IS_ENABLED(CONFIG_OPLUS_FEATURE_JANK_INFO)
...
struct oplus_task_struct {
	...
	u64 last_wake_ts;
	int fbg_cur_group;
#ifdef CONFIG_OPLUS_SCHED_LOCKING_PROTECT
...
#endif
#if IS_ENABLED(CONFIG_ARM64_AMU_EXTN) && IS_ENABLED(CONFIG_OPLUS_FEATURE_JANK_INFO)
	struct uid_struct *uid_struct;
	u64 amu_instruct;
	u64 amu_cycle;
#endif
...
static inline void init_task_ux_info(struct task_struct *t)
{
	...
	ots->abnormal_flag = 0;
#endif
#ifdef CONFIG_OPLUS_SCHED_SPREAD
	ots->lb_state = 0;
	ots->ld_flag = 0;
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_SCHED_BAN_APP_SET_AFFINITY)
