...
#include "../walt/walt_kallsyms.h"
#include "../walt/walt_cfs.h"
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CPUFREQ_EFFIENCY)
#include <linux/cpufreq_effiency.h>
#endif
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CPUFREQ_BOUNCING)
...
#if IS_ENABLED(OPLUS_FEATURE_CPUFREQ_HEALTH)
#include <linux/cpufreq_health.h>
#endif

#include "uag_systrace_debug.h"
#if IS_ENABLED(CONFIG_OPLUS_EAS_CPUFREQ_IOWAIT_PROTECT)
#include "../eas_opt/oplus_iowait.h"
#endif

#if IS_ENABLED(CONFIG_OPLUS_EAS_FAKE_CAP)
#include "../eas_opt/fake_cap.h"
#endif

#ifdef CONFIG_OPLUS_UAG_MULTI_LV_TL
...
#ifdef CONFIG_OPLUS_UAG_USE_TL
#ifdef CONFIG_OPLUS_UAG_NONLINEAR_FREQ_CTL
...
#endif /* CONFIG_OPLUS_UAG_NONLINEAR_FREQ_CTL */
				...
				unsigned long nl, unsigned long *util, unsigned long *max)
{
	...
	if (is_hiload && !is_migration)
		max_and_reason(util, sg_policy->hispeed_util);

	if (is_hiload && nl >= mult_frac(cpu_util, NL_RATIO, 100))
		max_and_reason(util, *max);
...
static int uag_gov_init(struct cpufreq_policy *policy)
{
    ...
    ret = ensure_sched_walt_once();
    if (ret) {
        pr_err("Failed to acquire sched_walt reference, aborting governor init.\n");
        return ret;
	}

    ret = resolve_walt_kallsyms_some_once();
    if (ret) {
        pr_err("Failed to resolve WALT symbols, aborting governor init.\n");
        return ret;
    }

	// ret = hook_walt_compute_energy_once();
 //    if (ret) {
 //        pr_err("Failed to hook walt_compute_energy, aborting governor init.\n");
 //        return ret;
 //    }
...
#ifdef CONFIG_OPLUS_UAG_TOUCH_BOOST
	touch_boost_init();
#endif
...
static void uag_gov_limits(struct cpufreq_policy *policy)
{
	...
	if (!policy->fast_switch_enabled) {
		 ...
		 * (2) cpufreq_set_policy to call governor->limits here
		 * so we have several times here and we must to keep them same
		 * here we using walt_ktime_get_ns() to keep same with walt scheduler
		 */
...
#if defined(CONFIG_OPLUS_UAG_NONLINEAR_FREQ_CTL) && defined(CONFIG_ARCH_MEDIATEK)
