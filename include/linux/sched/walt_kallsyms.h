...
#ifndef _LINUX_SCHED_WALT_KALLSYMS_H
#define _LINUX_SCHED_WALT_KALLSYMS_H
...
KSYM_FUNC(int, sched_lpm_disallowed_time, (int cpu, u64 *timeout), (cpu, timeout));
KSYM_FUNC(int, set_task_boost, (int boost, u64 period), (boost, period));

struct notifier_block;
KSYM_FUNC_VOID(core_ctl_notifier_register, (struct notifier_block *n), (n));
KSYM_FUNC_VOID(core_ctl_notifier_unregister, (struct notifier_block *n), (n));
KSYM_FUNC(int, core_ctl_set_boost, (bool boost), (boost));
KSYM_FUNC(int, walt_pause_cpus, (struct cpumask *cpus), (cpus));
KSYM_FUNC(int, walt_resume_cpus, (struct cpumask *cpus), (cpus));

RSYM_RESOLVER(resolve_sched_walt_kallsyms_all,
    RSYM(sched_lpm_disallowed_time),
    RSYM(set_task_boost),
    RSYM(core_ctl_notifier_register),
    RSYM(core_ctl_notifier_unregister),
    RSYM(core_ctl_set_boost),
    RSYM(walt_pause_cpus),
    RSYM(walt_resume_cpus),
);

MODULE_DEP(sched_walt);
...
static inline int resolve_sched_walt_kallsyms_all(void)
{
	return 0;
}
static inline int ensure_sched_walt(void)
{
	return 0;
}
static inline int ensure_sched_walt_once(void)
{
	return 0;
}
static inline int release_sched_walt(void)
{
	return 0;
}
#endif

#endif /* _LINUX_SCHED_WALT_KALLSYMS_H */
